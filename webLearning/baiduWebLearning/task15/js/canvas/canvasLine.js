	...
	if(canvas.getContext){
		...
		for(let i=0;i<dataArr.length;i++){//数据点
			for(let j=0;j<dataArr[i].length;j++){
				...
				ctx.arc(70+j*40,220-dataArr[i][j],5,0,(Math.PI)*2,true);
				ctx.fill();
				if(j!==dataArr[i].length-1){//连线，最后一个不用连
					ctx.beginPath();
					ctx.moveTo(70+j*40,220-dataArr[i][j]);
					ctx.lineTo(70+(1+j)*40,220-dataArr[i][j+1]);
					ctx.stroke();
				};
			};
		};
		...
		ctx.lineTo(30,5);
		ctx.lineTo(30,220);
		ctx.lineTo(40*(len+2),220);
		ctx.lineTo(40*(len+2)-5,220-5);
		ctx.lineTo(40*(len+2),220);
		ctx.lineTo(40*(len+2)-5,220+5);
		ctx.stroke();
	}else{
